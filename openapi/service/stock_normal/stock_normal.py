...
# 5.1.1. Place order
def place_order(request_dto, account_no):
...
# 5.2.1. Update order
def update_order(request_dto, account_no, order_id):
...
# 5.3.1. Cancel order
def cancel_order(account_no, request_dto):
...
def get_order(account_no, order_id):
...
def get_command_match_information(account_no):
    url = f"{Constant.BASE_URL_PRODUCTION}/aion/v1/accounts/{account_no}/orders"
    headers = RequestApi.get_headers(Constant.token)
    response = requests.get(url, headers=headers)
...
def get_purchasing_power(account_no):
...
def get_purchasing_power_by_symbol(account_no, symbol):
...
def get_purchasing_power_by_symbol_and_price(account_no, symbol, price):
...
def get_asset_stock_by_sub_account(account_no):
...
def get_cash_investment(account_no):
